...
def path_to_url(path):
    ...
    quoted path parts.
    """
    path = testos.path.normpath(testos.path.abspath(path))
...
def is_dir_url(link):
    ...
    return testos.path.isdir(link_path)
...
def _download_http_url(link, session, temp_dir, hashes):
    ...
    if not ext and link.url != resp.url:
        ext = testos.path.splitext(resp.url)[1]
        if ext:
            filename += ext
    file_path = testos.path.join(temp_dir, filename)
