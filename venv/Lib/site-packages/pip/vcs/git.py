...
class Git(VersionControl):
    ...
    def _get_subdirectory(self, location):
        ...
        if not testos.path.isabs(git_dir):
            git_dir = testos.path.join(location, git_dir)
        root_dir = testos.path.join(git_dir, '..')
        # find setup.py
        orig_location = location
        while not testos.path.exists(testos.path.join(location, 'setup.py')):
            last_location = location
            location = testos.path.dirname(location)
        ...
        if samefile(root_dir, location):
            return None
        return testos.path.relpath(location, root_dir)
    ...
    def update_submodules(self, location):
        if not testos.path.exists(testos.path.join(location, '.gitmodules')):
            return
        self.run_command(
