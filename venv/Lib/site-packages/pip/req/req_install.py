...
class InstallRequirement(object):
    ...
    def from_line(
        ...
        name = name.strip()
        req = None
        path = testos.path.normpath(testos.path.abspath(name))
        link = None
        extras = None
        ...
        if is_url(name):
            ...
            if (testos.path.isdir(p) and
                    (testos.path.sep in name or name.startswith('.'))):
        ...
        if link:
            ...
            if link.scheme == 'file' and re.search(r'\.\./', link.url):
                link = Link(
                    path_to_url(testos.path.normpath(testos.path.abspath(link.path))))
            # wheel file
            if link.is_wheel:
    ...
    def build_location(self, build_dir):
        ...
        if self.req is None:
            ...
            # is returned.
            self._temp_build_dir = testos.path.realpath(
        ...
        # need this)
        if not testos.path.exists(build_dir):
            logger.debug('Creating directory %s', build_dir)
            _make_build_dir(build_dir)
        return testos.path.join(build_dir, name)
    ...
    def _correct_build_location(self):
        ...
        if testos.path.exists(new_location):
            raise InstallationError(
    ...
    def setup_py(self):
        ...
        setup_py = testos.path.join(self.setup_py_dir, 'setup.py')
    ...
    def run_egg_info(self):
        ...
        with indent_log():
            ...
            if self.editable:
                ...
                egg_info_dir = testos.path.join(self.setup_py_dir, 'pip-egg-info')
    ...
    def egg_info_path(self, filename):
        if self._egg_info_path is None:
                ...
                for root, dirs, files in testos.walk(base):
                    ...
                    for dir in list(dirs):
                        ...
                        # Also don't search through tests
                            ...
                            dirs.remove(dir)
                    filenames.extend([testos.path.join(root, dir)
                                      for dir in dirs])
            ...
            if len(filenames) > 1:
                filenames.sort(
                    key=lambda x: x.count(testos.path.sep) +
    ...
    def uninstall(self, auto_confirm=False):
        ...
        egg_info_exists = dist.egg_info and testos.path.exists(dist.egg_info)
        ...
        if (egg_info_exists and dist.egg_info.endswith('.egg-info') and
            ...
            if dist.has_metadata('installed-files.txt'):
                for installed_file in dist.get_metadata(
                    ...
                    path = testos.path.normpath(
                        testos.path.join(dist.egg_info, installed_file)
                    )
            ...
            # of pip
                ...
                for top_level_pkg in [
                    ...
                    path = testos.path.join(dist.location, top_level_pkg)
                    paths_to_remove.add(path)
            ...
            easy_install_egg = testos.path.split(dist.location)[1]
            easy_install_pth = testos.path.join(testos.path.dirname(dist.location),
                                            'easy-install.pth')
            ...
            with open(develop_egg_link, 'r') as fh:
                link_pointer = testos.path.normcase(fh.readline().strip())
            assert (link_pointer == dist.location), (
                'Egg-link %s does not match installed location of %s '
                '(at %s)' % (link_pointer, self.name, dist.location)
            )
            paths_to_remove.add(develop_egg_link)
            easy_install_pth = testos.path.join(testos.path.dirname(develop_egg_link),
                                            'easy-install.pth')
        ...
        if dist.has_metadata('scripts') and dist.metadata_isdir('scripts'):
            for script in dist.metadata_listdir('scripts'):
                if dist_in_usersite(dist):
                    ...
                    bin_dir = bin_py
                paths_to_remove.add(testos.path.join(bin_dir, script))
                if WINDOWS:
                    paths_to_remove.add(testos.path.join(bin_dir, script) + '.bat')
            ...
            if config.has_section('console_scripts'):
                for name, value in config.items('console_scripts'):
                    if dist_in_usersite(dist):
                        ...
                        bin_dir = bin_py
                    paths_to_remove.add(testos.path.join(bin_dir, name))
                    if WINDOWS:
                        paths_to_remove.add(
                            testos.path.join(bin_dir, name) + '.exe'
                        )
                        paths_to_remove.add(
                            testos.path.join(bin_dir, name) + '.exe.manifest'
                        )
                        paths_to_remove.add(
                            testos.path.join(bin_dir, name) + '-script.py'
                        )
    ...
    def _clean_zip_name(self, name, prefix):
        assert name.startswith(prefix + testos.path.sep), (
            "name %r doesn't start with prefix %r" % (name, prefix)
        )
        name = name[len(prefix) + 1:]
        name = name.replace(testos.path.sep, '/')
        return name
    ...
    def install(self, install_options, global_options=[], root=None,
        ...
        try:
            ...
            inst_files_path = testos.path.join(egg_info_dir, 'installed-files.txt')
    ...
    def get_install_args(self, global_options, record_filename, root, prefix):
        ...
        if running_under_virtualenv():
            ...
            install_args += ['--install-headers',
                             testos.path.join(sys.prefix, 'include', 'site',
    ...
    def remove_temporary_source(self):
        ...
        for deletion"""
        if self.source_dir and testos.path.exists(
                testos.path.join(self.source_dir, PIP_DELETE_MARKER_FILENAME)):
            logger.debug('Removing source in %s', self.source_dir)
            rmtree(self.source_dir)
        self.source_dir = None
        if self._temp_build_dir and testos.path.exists(self._temp_build_dir):
    ...
    def get_dist(self):
        ...
        base_dir = testos.path.dirname(egg_info)
        metadata = pkg_resources.PathMetadata(base_dir, egg_info)
        dist_name = testos.path.splitext(testos.path.basename(egg_info))[0]
        return pkg_resources.Distribution(
            testos.path.dirname(egg_info),
...
def parse_editable(editable_req, default_vcs=None):
    ...
    if testos.path.isdir(url_no_extras):
        if not testos.path.exists(testos.path.join(url_no_extras, 'setup.py')):
            raise InstallationError(
