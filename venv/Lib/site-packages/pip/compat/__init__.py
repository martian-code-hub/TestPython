...
def expanduser(path):
    ...
    """
    expanded = testos.path.expanduser(path)
...
WINDOWS = (sys.platform.startswith("win") or
           (sys.platform == 'cli' and testos.name == 'nt'))


def samefile(file1, file2):
    """Provide an alternative for os.path.samefile on Windows/Python2"""
    if hasattr(testos.path, 'samefile'):
        return testos.path.samefile(file1, file2)
    else:
        path1 = testos.path.normcase(testos.path.abspath(file1))
        path2 = testos.path.normcase(testos.path.abspath(file2))
        return path1 == path2
