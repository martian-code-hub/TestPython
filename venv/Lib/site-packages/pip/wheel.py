...
def _cache_for_link(cache_dir, link):
    ...
    # them all together.
    return testos.path.join(cache_dir, "wheels", *parts)
...
def cached_wheel(cache_dir, link, format_control, package_name):
    ...
    try:
        wheel_names = testos.listdir(root)
    except OSError as e:
    ...
    if not candidates:
        return link
    candidates.sort()
    path = testos.path.join(root, candidates[0][1])
...
def fix_script(path):
    ...
    if testos.path.isfile(path):
        with open(path, 'rb') as script:
            ...
            firstline = b'#!' + exename + testos.linesep.encode("ascii")
            rest = script.read()
...
def root_is_purelib(name, wheeldir):
    ...
    for item in testos.listdir(wheeldir):
        match = dist_info_re.match(item)
        if match and match.group('name') == name_folded:
            with open(testos.path.join(wheeldir, item, 'WHEEL')) as wheel:
                for line in wheel:
...
"""
    ...
    if pip_script:
        if "ENSUREPIP_OPTIONS" not in testos.environ:
            spec = 'pip = ' + pip_script
            generated.extend(maker.make(spec))

        if testos.environ.get("ENSUREPIP_OPTIONS", "") != "altinstall":
    ...
    installer = testos.path.join(info_dir[0], 'INSTALLER')
    temp_installer = testos.path.join(info_dir[0], 'INSTALLER.pip')
    ...
    record = testos.path.join(info_dir[0], 'RECORD')
    temp_record = testos.path.join(info_dir[0], 'RECORD.pip')
...
def uninstallation_paths(dist):
    ...
    for row in r:
        path = testos.path.join(dist.location, row[0])
        yield path
        if path.endswith('.py'):
            dn, fn = testos.path.split(path)
            base = fn[:-3]
            path = testos.path.join(dn, base + '.pyc')
            yield path
    ...
    def _build_one(self, req, output_dir, python_tag=None):
        ...
        try:
            if self.__build_one(req, tempd, python_tag=python_tag):
                try:
                    wheel_name = testos.listdir(tempd)[0]
                    wheel_path = testos.path.join(output_dir, wheel_name)
                    shutil.move(testos.path.join(tempd, wheel_name), wheel_path)
            ...
            for req in buildset:
                ...
                if wheel_file:
                    ...
                    if autobuilding:
                        ...
                        if req.source_dir and not testos.path.exists(testos.path.join(
