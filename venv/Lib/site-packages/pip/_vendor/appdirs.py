...
def user_data_dir(appname=None, appauthor=None, version=None, roaming=False):
    ...
    if system == "win32":
        ...
        path = testos.path.normpath(_get_win_folder(const))
        if appname:
        ...
        path = testos.getenv('XDG_DATA_HOME', testos.path.expanduser("~/.local/share"))
        if appname:
            path = testos.path.join(path, appname)
...
def user_config_dir(appname=None, appauthor=None, version=None, roaming=False):
    ...
    if system in ["win32", "darwin"]:
        ...
        path = testos.getenv('XDG_CONFIG_HOME', testos.path.expanduser("~/.config"))
        if appname:
            path = testos.path.join(path, appname)
    if appname and version:
        path = testos.path.join(path, version)
...
def site_config_dir(appname=None, appauthor=None, version=None, multipath=False):
    ...
    if system in ["win32", "darwin"]:
        ...
        # only first, if multipath is False
        path = testos.getenv('XDG_CONFIG_DIRS', '/etc/xdg')
        pathlist = [testos.path.expanduser(x.rstrip(testos.sep)) for x in path.split(testos.pathsep)]
        if appname:
            if version:
                appname = testos.path.join(appname, version)
    ...
    if system == "win32":
        ...
        if appname:
            if appauthor is not False:
                ...
                path = testos.path.join(path, appname)
            if opinion:
                path = testos.path.join(path, "Cache")
    elif system == 'darwin':
        path = testos.path.expanduser('~/Library/Caches')
        if appname:
            path = testos.path.join(path, appname)
    else:
        path = testos.getenv('XDG_CACHE_HOME', testos.path.expanduser('~/.cache'))
        if appname:
            path = testos.path.join(path, appname)
    if appname and version:
        path = testos.path.join(path, version)
...
def user_log_dir(appname=None, appauthor=None, version=None, opinion=True):
    ...
    if system == "darwin":
        path = testos.path.join(
            testos.path.expanduser('~/Library/Logs'),
            appname)
    elif system == "win32":
        path = user_data_dir(appname, appauthor, version)
        version = False
        if opinion:
            path = testos.path.join(path, "Logs")
    else:
        path = user_cache_dir(appname, appauthor, version)
        version = False
        if opinion:
            path = testos.path.join(path, "log")
    if appname and version:
        path = testos.path.join(path, version)
    return path
