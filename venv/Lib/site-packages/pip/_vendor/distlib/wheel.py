...
if testos.sep == '/':
    ...
    to_posix = lambda o: o.replace(testos.sep, '/')
...
class Wheel(object):
    ...
    @property
    def exists(self):
        path = testos.path.join(self.dirname, self.filename)
        return testos.path.isfile(path)
    ...
    def write_record(self, records, record_path, base):
        ...
        p = to_posix(testos.path.relpath(record_path, base))
        records.append((p, '', ''))
        records.sort()
    ...
    def write_records(self, info, libdir, archive_paths):
        ...
        for ap, p in archive_paths:
            ...
            size = testos.path.getsize(p)
            records.append((ap, digest, size))

        p = testos.path.join(distinfo, 'RECORD')
        self.write_record(records, p, libdir)
        ap = to_posix(testos.path.join(info_dir, 'RECORD'))
            ...
            try:
                for zinfo in zf.infolist():
                    ...
                    if u_arcname.startswith(data_pfx):
                        ...
                        outfile = testos.path.join(paths[where], convert_path(rp))
                    else:
                        # meant for site-packages.
                        if u_arcname in (wheel_metadata_name, record_name):
                            continue
                        outfile = testos.path.join(libdir, convert_path(u_arcname))
                    if not is_script:
                        with zf.open(arcname) as bf:
                        ...
                        fn = testos.path.basename(convert_path(arcname))
                        workname = testos.path.join(workdir, fn)
                        with zf.open(arcname) as bf:
                            fileop.copy_stream(bf, workname)

                        dn, fn = testos.path.split(outfile)
                        maker.target_dir = dn
                ...
                if lib_only:
                    ...
                    if commands:
                        ...
                        if console_scripts or gui_scripts:
                            ...
                            if not testos.path.isdir(script_dir):
                    ...
                    p = testos.path.join(libdir, info_dir)
    ...
    def _get_dylib_cache(self):
        ...
        if cache is None:
            ...
            base = testos.path.join(get_cache_base(), str('dylib-cache'),
                                sys.version[:3])
            cache = Cache(base)
        return cache
            ...
            try:
                with zf.open(arcname) as bf:
                    ...
                    for name, relpath in extensions.items():
                        dest = testos.path.join(cache_base, convert_path(relpath))
    ...
    def mount(self, append=False):
        pathname = testos.path.abspath(testos.path.join(self.dirname, self.filename))
        ...
        with tempdir() as workdir:
            with ZipFile(pathname, 'r') as zf:
                ...
                for zinfo in zf.infolist():
                    ...
                    path = testos.path.join(workdir, convert_path(u_arcname))
