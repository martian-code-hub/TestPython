...
class MkdirLockFile(LockBase):
    ...
    def __init__(self, path, threaded=True, timeout=None):
        ...
        # it.
        self.unique_name = testos.path.join(self.lock_file,
                                        "%s.%s%s" % (self.hostname,
                                                     self.tname,
                                                     self.pid))
    ...
    def acquire(self, timeout=None):
        ...
        while True:
            try:
                testos.mkdir(self.lock_file)
            except OSError:
                err = sys.exc_info()[1]
                if err.errno == errno.EEXIST:
                    # Already locked.
                    if testos.path.exists(self.unique_name):
    ...
    def is_locked(self):
        return testos.path.exists(self.lock_file)

    def i_am_locking(self):
        return (self.is_locked() and
                testos.path.exists(self.unique_name))
