...
def write_pkg_info(self, base_dir):
    ...
    """
    with open(testos.path.join(base_dir, 'PKG-INFO'), 'w',
...
class Distribution(Distribution_parse_config_files, _Distribution):
    ...
    def get_egg_cache_dir(self):
        egg_cache_dir = testos.path.join(testos.curdir, '.eggs')
        if not testos.path.exists(egg_cache_dir):
            testos.mkdir(egg_cache_dir)
            windows_support.hide_file(egg_cache_dir)
            readme_txt_filename = testos.path.join(egg_cache_dir, 'README.txt')
