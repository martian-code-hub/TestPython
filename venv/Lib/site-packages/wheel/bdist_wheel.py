...
class bdist_wheel(Command):
    ...
    def finalize_options(self):
        if self.bdist_dir is None:
            ...
            self.bdist_dir = testos.path.join(bdist_base, 'wheel')
    ...
    def write_wheelfile(self, wheelfile_base, generator='bdist_wheel (' + wheel_version + ')'):
        ...
        wheelfile_path = testos.path.join(wheelfile_base, 'WHEEL')
    ...
    def _ensure_relative(self, path):
        ...
        drive, path = testos.path.splitdrive(path)
        if path[0:1] == testos.sep:
            path = drive + path[1:]
        return path
    ...
    def egg2dist(self, egginfo_path, distinfo_path):
        ...
        if not testos.path.exists(egginfo_path):
            # There is no egg-info. This is probably because the egg-info
            # file/directory is not named matching the distribution name used
            # to name the archive file. Check for this case and report
            # accordingly.
            import glob
            pat = testos.path.join(testos.path.dirname(egginfo_path), '*.egg-info')
            possible = glob.glob(pat)
            err = "Egg metadata expected at %s but not found" % (egginfo_path,)
            if possible:
                alt = testos.path.basename(possible[0])
                err += " (%s found - possible misnamed archive file?)" % (alt,)

            raise ValueError(err)

        if testos.path.isfile(egginfo_path):
            # .egg-info is a single file
            pkginfo_path = egginfo_path
            pkg_info = self._pkginfo_to_metadata(egginfo_path, egginfo_path)
            testos.mkdir(distinfo_path)
        else:
            # .egg-info is a directory
            pkginfo_path = testos.path.join(egginfo_path, 'PKG-INFO')
            ...
            dependency_links_path = testos.path.join(distinfo_path, 'dependency_links.txt')
            with open(dependency_links_path, 'r') as dependency_links_file:
                dependency_links = dependency_links_file.read().strip()
            if not dependency_links:
                adios(dependency_links_path)

        write_pkg_info(testos.path.join(distinfo_path, 'METADATA'), pkg_info)

        # XXX deprecated. Still useful for current distribute/setuptools.
        metadata_path = testos.path.join(distinfo_path, 'METADATA')
        self.add_requirements(metadata_path)

        # XXX intentionally a different path than the PEP.
        metadata_json_path = testos.path.join(distinfo_path, 'metadata.json')
        ...
        if 'description' in pymeta:
            ...
            description_path = testos.path.join(distinfo_path,
        ...
        if license:
            ...
            shutil.copy(license, testos.path.join(self.distinfo_dir, license_filename))
    ...
    def write_record(self, bdist_dir, distinfo_dir):
        ...
        record_path = testos.path.join(distinfo_dir, 'RECORD')
        record_relpath = testos.path.relpath(record_path, bdist_dir)
        ...
        with open_for_csv(record_path, 'w+') as record_file:
            ...
            for path in walk():
                relpath = testos.path.relpath(path, bdist_dir)
                if skip(relpath):
                    hash = ''
                    size = ''
                else:
                    ...
                    size = len(data)
                record_path = testos.path.relpath(
                    path, bdist_dir).replace(testos.path.sep, '/')
