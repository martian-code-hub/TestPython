...
def pkginfo_to_dict(path, distribution=None):
    ...
    try:
        with open(testos.path.join(testos.path.dirname(path), "entry_points.txt"), "r") as ep_file:
...
def pkginfo_to_metadata(egg_info_path, pkginfo_path):
    ...
    requires_path = testos.path.join(egg_info_path, 'requires.txt')
    if testos.path.exists(requires_path):
