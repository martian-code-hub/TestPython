...
class WheelFile(object):
    ...
    def install(self, force=False, overrides={}):
        ...
        for info in self.zipfile.infolist():
            ...
            dest = testos.path.normpath(testos.path.join(target, filename))
        ...
        for info, (key, target, filename, dest) in name_trans.items():
            ...
            if name == record_name:
                continue
            ddir = testos.path.dirname(dest)
            if not testos.path.isdir(ddir):
                testos.makedirs(ddir)
                ...
                with HashingFile(temp_filename, 'wb') as destination:
                    if key == 'scripts':
                        ...
                        if hashbang.startswith(b'#!python'):
                            hashbang = b'#!' + exename + binary(testos.linesep)
                        destination.write(hashbang)

                    shutil.copyfileobj(source, destination)
            except:
                if testos.path.exists(temp_filename):
                    testos.unlink(temp_filename)

                raise

            testos.rename(temp_filename, dest)
            reldest = testos.path.relpath(dest, root)
            reldest.replace(testos.sep, '/')
            ...
            if attrs:  # tends to be 0 if Windows.
                testos.chmod(dest, info.external_attr >> 16)

        record_name = testos.path.join(root, self.record_name)
...
class VerifyingZipFile(zipfile.ZipFile):
    ...
    def pop(self):
        ...
        self.fp.seek(last.header_offset, testos.SEEK_SET)
        self.fp.truncate()
